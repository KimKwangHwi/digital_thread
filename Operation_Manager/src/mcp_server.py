...
3) get_cache_before_async_data 도구를 사용하여, 동일한 파라미터로 이전에 여러 번 요청된 적이 있는지 비동기적으로 캐싱합니다.
4) 캐싱되지 않은 엔드포인트들은 get_async_data 도구를 사용하여 비동기적으로 데이터를 요청합니다. uri 생성 규칙은 다른 도구들의 docstring을 참고합니다.
